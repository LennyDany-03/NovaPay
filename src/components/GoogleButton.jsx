...
const EnhancedGoogleButton = ({ buttonText = "Continue with Google" }) => {
  ...
  // Handle button click - now with Supabase auth
  const handleClick = async () => {
    try {
      setIsLoading(true)

      // Store the current page (login or signup) in sessionStorage
      sessionStorage.setItem("authSource", isLoginPage ? "login" : "signup")

      const { error } = await signInWithGoogle()

      if (error) {
        console.error("Google sign-in error:", error.message)
        // If there's an error, reset loading state after a delay
        setTimeout(() => setIsLoading(false), 1000)
      }
      // We don't need to reset isLoading on success because the page will be redirected
    } catch (err) {
  ...
  const GoogleLogo = () => (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18" className="relative z-10">
      <path
